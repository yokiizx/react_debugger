...
function beginWork(current, workInProgress, renderLanes) {
  ...
  if (current !== null) {
    ...
    var newProps = workInProgress.pendingProps; // 根据 props和type的对比来判断是否可以复用

    if ( // 1 props
    oldProps !== newProps || hasContextChanged() || ( // Force a re-render if the implementation changed due to hot reload:
    // 2 type
     workInProgress.type !== current.type )) {
